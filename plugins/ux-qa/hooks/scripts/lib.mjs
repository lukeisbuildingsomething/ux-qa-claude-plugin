...
export function nudgeText(files, root) {
  ...
  return [
    ...
    '  - Fold any decisions already buffered in .claude/ux-qa/DECISIONS.md into',
    '    this turn, then ask the top four open decisions in ONE AskUserQuestion',
    '    call at the end of the turn. Several questions in one call is correct.',
    '    Drain that buffer whenever the user is present to answer.',
    '  - Stay silent only if nobody is there to answer, or the user is mid-flow',
    '    on something unrelated and urgent. Low severity is not a reason to wait.',
    '  - The silence rule, in every step: NEVER tell the user to run a slash',
    '    command, NEVER name a ux-qa file to them, and NEVER report bookkeeping',
    '    state — line counts, queue depth, fix counts, when the last audit ran.',
    '    If it is worth mentioning, ask it as a question instead.',
    '  - Say at most one sentence before the questions.',
